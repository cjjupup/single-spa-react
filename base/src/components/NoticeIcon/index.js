...
export default class NoticeIcon extends PureComponent {
  ...
  getNotificationBox() {
    ...
    return (
      <Spin spinning={loading} delay={0}>
        <Tabs className={style.tabs} onChange={this.onTabChange}>
          {panes}
        </Tabs>
      </Spin>
    )
  ...
  render() {
    ...
    const noticeButtonClass = classNames(className, style.noticeButton)
    const notificationBox = this.getNotificationBox()
    const trigger = (
      <span className={noticeButtonClass}>
        <Badge count={count} className={style.badge}>
