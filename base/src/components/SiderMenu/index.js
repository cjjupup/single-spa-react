...
const getIcon = icon => {
  if (typeof icon === 'string' && icon.indexOf('http') === 0) {
    return <img src={icon} alt='icon' className={`${style.icon} ${style['sider-menu-item-img']}`} />
  }
...
export default class SiderMenu extends Component {
  ...
  // 包装点击事件
  clickEvent (item) {
    window.baseHistory.tabsStore.openTab(item)
  }
  render() {
    ...
    return (
      <Sider
        ...
        width={220}
        className={`${style.sider} other-ele`}
      >
        <div className={style.logo} key='logo'>
          <Link to='/'>
            {/* <img src={logo} alt='logo' /> */}
            <h1 className={style.hahaha}>Ant Design Pro</h1>
          </Link>
        </div>
        <Menu
          key='Menu'
